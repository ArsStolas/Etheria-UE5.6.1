...
public:
    ...
    UFUNCTION(BlueprintCallable, Category="Rope|Attach|Visual")
...
protected:
    ...
    UPROPERTY(VisibleAnywhere, Category="Rope|Attach|Visual")
    UCableComponent* CableComponent = nullptr;

    UPROPERTY(EditAnywhere, Category="Rope|Attach|Visual")
    float RopeWidth = 5.f;
    ...
    UPROPERTY(EditAnywhere, Category="Rope|Attach")
    float AnchorAttachmentOffset = 25.f;
    
    // Min/max width for cable visual
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Rope|Attach|Visual")
    float MinRopeWidth = 2.f;
    
    UPROPERTY(EditAnywhere, Category="Rope|Attach|Pull")
    ...
    UPROPERTY(EditAnywhere, Category="Rope|Attach|Climb")
    ...
    UPROPERTY(EditAnywhere, Category="Rope|Attach|Physics", meta=(ClampMin="5", ClampMax="10"))
    int32 NumSegments = 10;
    
    /** Substep time for physics simulation (lower = more stable) */
    UPROPERTY(EditAnywhere, Category="Rope|Attach|Physics", meta=(ClampMin="0.01", ClampMax="0.1"))
    float SubstepTime = 0.02f;
    
    /** Solver iterations (higher = less stretchy but more expensive) */
    UPROPERTY(EditAnywhere, Category="Rope|Attach|Physics", meta=(ClampMin="1", ClampMax="10"))
    int32 SolverIterations = 2;
    
    /** Damping force applied to cable to reduce bouncing (higher = less bouncy) */
    UPROPERTY(EditAnywhere, Category="Rope|Attach|Physics", meta=(ClampMin="0", ClampMax="100"))
    FVector CableForce = FVector(0.f, 0.f, -50.f);
    
    /** Cable gravity scale (1.0 = normal gravity, adjust for rope weight feel) */
    UPROPERTY(EditAnywhere, Category="Rope|Attach|Physics", meta=(ClampMin="0", ClampMax="3"))
    float CableGravityScale = 1.0f;
    
    /** Enable collision for cable with world geometry */
    UPROPERTY(EditAnywhere, Category="Rope|Attach|Physics")
    bool bEnableCollision = true; // ACTIVÉ par défaut maintenant
    
    /** Collision radius for each cable segment */
    UPROPERTY(EditAnywhere, Category="Rope|Attach|Physics", meta=(ClampMin="1", ClampMax="20"))
