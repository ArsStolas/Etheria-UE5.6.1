...
void SMyQuest::Construct(const FArguments& InArgs, TWeakPtr<FQuestBuilderEditor> InQuestEditor, const UQuestBuilderGraph* InQuestSystemGraph)
{
	bNeedsRefresh = false;
	...
	{
		QuestBuilderGraph = QuestEditorPtr.Pin()->GetQuestBuilderGraph();
	...
	else
	{
		...
		QuestBuilderGraph = const_cast<UQuestBuilderGraph*>(InQuestSystemGraph);
		check(QuestBuilderGraph);
	...
	ExpandedSections.Add(QuestSectionID::QUESTLIST, true);
...
void SMyQuest::Refresh()
{
	...
	//FBlueprintEditorUtils::ConformImplementedInterfaces(GetQuestBuilderGraph());
...
{
	return QuestEditorPtr.IsValid() ? SNew(SQuestPaletteItem, InCreateData, QuestEditorPtr.Pin()) : SNew(SQuestPaletteItem, InCreateData, GetQuestBuilderGraph());
}

void SMyQuest::CollectAllActions(FGraphActionListBuilderBase& OutAllActions)
{
	UQuestBuilderGraph* QuestSystemObj = GetQuestBuilderGraph();
	check(QuestSystemObj);

	TSharedPtr<FQuestBuilderEditor> QuestSystemEditor = QuestEditorPtr.Pin();
	...
	if (QuestSystemEditor.IsValid())
	{
		...
		for (int32 i = 0; i < QuestSystemObj->QuestGraphPages.Num(); i++)
		{
			...
			TSharedPtr<FAssetSchemaAction_QuestSystemGraph> NeUbergraphAction = MakeShareable(new FAssetSchemaAction_QuestSystemGraph(FText::GetEmpty(), DisplayInfo.PlainName, DisplayInfo.Tooltip, 2, QuestSectionID::QUESTLIST));
...
{
	StaticSectionIDs.Add(QuestSectionID::QUESTLIST);
}
...
void SMyQuest::OnActionSelected(const TArray<TSharedPtr<FEdGraphSchemaAction>>& InActions)
{
	...
	if (InAction.IsValid())
	{
		...
		{
			TSharedPtr<FQuestBuilderEditor> QuestEditor = QuestEditorPtr.Pin();

			if (UQuestBuilderEdGraph* QuestGraph = Cast<UQuestBuilderEdGraph>(QuestEditor->FocusedEdGraph))
			{
				QuestEditor->PropertyWidget->SetObject(QuestGraph->Quest);
			}
		}
	}
}
...
void SMyQuest::ExecuteAction(TSharedPtr<FEdGraphSchemaAction> InAction)
{
	...
	UQuestBuilderGraph* QuestSystemObj = QuestEditorPtr.Pin()->GetQuestBuilderGraph();
	if (InAction.IsValid())
	{
		if (InAction->GetTypeId() == FAssetSchemaAction_QuestSystemGraph::StaticGetTypeId())
		{
			...
			if (GraphAction->EdGraph)
			{
				...
				if (UQuestBuilderEdGraph* QuestGraph = Cast<UQuestBuilderEdGraph>(QuestEditor->CurrentGraphWidget->GetCurrentGraph()))
				{
					QuestEditor->PropertyWidget->SetObject(QuestGraph->Quest);
				}
			}
...
TSharedPtr<SWidget> SMyQuest::OnContextMenuOpening()
{
	...
	if (SelectionHasContextMenu())
	{
		MenuBuilder.BeginSection("BasicOperations");
		{
			...
			MenuBuilder.AddMenuEntry(FGenericCommands::Get().Rename, NAME_None, LOCTEXT("Rename", "Rename"), LOCTEXT("Rename_Tooltip", "Rename Quest"));
...
void SMyQuest::BuildAddNewMenu(FMenuBuilder& MenuBuilder)
{
	...
	if (UQuestBuilderGraph* CurrentBlueprint = GetQuestBuilderGraph())
	{
		MenuBuilder.AddMenuEntry(FQuestBuilder_EditorCommands::Get().AddNewQuestGraph);
...
FText SMyQuest::OnGetSectionTitle(int32 InSectionID)
{
	...
	switch (InSectionID)
	{
	case QuestSectionID::QUESTLIST:
		SeperatorTitle = NSLOCTEXT("GraphActionNode", "Quest List", "Quest List");
		break; 
	default:
...
FReply SMyQuest::OnAddButtonClickedOnSection(int32 InSectionID)
{
	...
	{
	case QuestSectionID::QUESTLIST:
		CommandList->ExecuteAction(FQuestBuilder_EditorCommands::Get().AddNewQuestGraph.ToSharedRef());
		break;
	}
...
void SMyQuest::OnDeleteGraph(UEdGraph* InGraph)
{
	if (InGraph && InGraph->bAllowDeletion)
	{
		...
		TArray<UQuestBuilderEdNode*> AllNodes;
		FQuestBuilderEditorUtils::GetAllNodesOfClass<UQuestBuilderEdNode>(GetQuestBuilderGraph(), AllNodes);

		const bool bDontRecompile = true;
		for (UQuestBuilderEdNode* CompNode : AllNodes)
		{
			if (CompNode->GetQuestBuilderEdGraph() == InGraph)
			{
				FQuestBuilderEditorUtils::RemoveNode(GetQuestBuilderGraph(), CompNode, bDontRecompile);
			}
		}
		

		FQuestBuilderEditorUtils::RemoveGraph(GetQuestBuilderGraph(), InGraph);
...
void SMyQuest::OpenGraph(FDocumentTracker::EOpenDocumentCause InCause, bool bOpenExternalGraphInNewEditor)
{
	...
	if (FAssetSchemaAction_QuestSystemGraph* GraphAction = SelectionAsGraph())
	{
		...
		/*if (!GraphToOpen)
		{
			GraphToOpen = FQuestBuilderEditorUtils::FindQuestGraph(GetQuestBuilderGraph());
		}*/
	}
...
{
	if (InObject == QuestBuilderGraph && (InPropertyChangedEvent.ChangeType != EPropertyChangeType::ValueSet && InPropertyChangedEvent.ChangeType != EPropertyChangeType::ArrayClear))
	{
		bNeedsRefresh = true;
	}
}
