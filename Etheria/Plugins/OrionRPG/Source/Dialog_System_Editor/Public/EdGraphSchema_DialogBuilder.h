...
#include "EdGraphSchema_DialogBuilder.generated.h"

namespace EDialogSubNode
{
	enum Type {
		Decorator,
		Event
...
class UDialogBuilderEdSubNode_Event;
class UDialogBuilderEdSubNode_Decorator;
...
UCLASS(MinimalAPI)
class UEdGraphSchema_DialogBuilder : public UEdGraphSchema
{
	GENERATED_BODY()
public:
	UEdGraphSchema_DialogBuilder();
	...
	TSubclassOf<UDialogBuilderEdSubNode_Decorator> DecoratorClass;
	TSubclassOf<UDialogBuilderEdSubNode_Event> EventClass;
protected:
	virtual FGraphNodeClassHelper& GetClassCache(int32 SubNodeFlags) const;
	static TSharedPtr<FAssetSchemaAction_DialogSystem_NewNode> AddNewNodeAction(FGraphActionListBuilderBase& ContextMenuBuilder, const FText& Category, const FText& MenuDesc, const FText& Tooltip);
	static TSharedPtr<FAssetSchemaAction_DialogSystem_NewSubNode> AddNewSubNodeAction(FGraphActionListBuilderBase& ContextMenuBuilder, const FText& Category, const FText& MenuDesc, const FText& Tooltip);
...
/** Action to add a node to the graph */
USTRUCT()
struct DIALOG_SYSTEM_EDITOR_API FAssetSchemaAction_DialogSystem_NewNode : public FEdGraphSchemaAction
{
	GENERATED_USTRUCT_BODY();

public:
	FAssetSchemaAction_DialogSystem_NewNode() : NodeTemplate(nullptr) {}

	FAssetSchemaAction_DialogSystem_NewNode(const FText& InNodeCategory, const FText& InMenuDesc, const FText& InToolTip, const int32 InGrouping)
		: FEdGraphSchemaAction(InNodeCategory, InMenuDesc, InToolTip, InGrouping), NodeTemplate(nullptr) {}

	virtual UEdGraphNode* PerformAction(class UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2D Location, bool bSelectNewNode = true) override;
	virtual void AddReferencedObjects(FReferenceCollector& Collector) override;

	TObjectPtr<UDialogBuilderEdNode> NodeTemplate;
};

USTRUCT()
struct DIALOG_SYSTEM_EDITOR_API FAssetSchemaAction_DialogSystem_NewEdge : public FEdGraphSchemaAction
{
	GENERATED_USTRUCT_BODY();

public:
	FAssetSchemaAction_DialogSystem_NewEdge() : NodeTemplate(nullptr) {}

	FAssetSchemaAction_DialogSystem_NewEdge(const FText& InNodeCategory, const FText& InMenuDesc, const FText& InToolTip, const int32 InGrouping)
		: FEdGraphSchemaAction(InNodeCategory, InMenuDesc, InToolTip, InGrouping), NodeTemplate(nullptr) {}
...
USTRUCT()
struct DIALOG_SYSTEM_EDITOR_API FAssetSchemaAction_DialogSystem_NewSubNode : public FEdGraphSchemaAction
{
	GENERATED_USTRUCT_BODY();

	/** Template of node we want to create */
	UPROPERTY()
	TObjectPtr<class UDialogBuilderEdNode> NodeTemplate;

	/** parent node */
	UPROPERTY()
	TObjectPtr<class UDialogBuilderEdNode> ParentNode;

	FAssetSchemaAction_DialogSystem_NewSubNode()
		: FEdGraphSchemaAction()
	...
	FAssetSchemaAction_DialogSystem_NewSubNode(FText InNodeCategory, FText InMenuDesc, FText InToolTip, const int32 InGrouping)
...
USTRUCT()
struct DIALOG_SYSTEM_EDITOR_API FDialogSchemaAction_AddComment : public FEdGraphSchemaAction
{
	GENERATED_BODY()

	FDialogSchemaAction_AddComment() : FEdGraphSchemaAction() {}
	FDialogSchemaAction_AddComment(FText InDescription, FText InToolTip)
